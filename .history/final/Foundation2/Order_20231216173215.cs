...
public class Order
{
    ...
    public double CalculateSubTotalPrice()
    {
        ...
        foreach (Product product in _products)
        {
            ...
            subTotalPrice = product.Sum(price => (price));
            // subPrice += price;
    ...
    {
        double shippingCost;
        string address = _customerAddress;
    ...
    public string GetTotalPrice()
    {
        ...
        //    totalCost += $"{product.GetName()}: {product.GetProductId()} - ${product.GetPrice()} x {product.GetQuantity()} = {product.TotalPrice()}\n";
            ...
            // total += product;
            subTotal = product.CalculateSubTotalPrice();
            shipping = product.GetShippingPrice();
            totalPrice = Sum(subTotal.shipping);           
        }
    ...
    public void DisplayDetails()
    {
        ...
        string totalCost = GetTotalPrice();
